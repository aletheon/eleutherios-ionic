...
export class UserForumService {
  ...
  public serviceIsServingInUserForumFromPromise (parentUserId: string, serviceId: string): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      const that = this;
      const forumRef = this.afs.collection<Forum>(`users/${parentUserId}/forums`);
          ...
          let promises = querySnapshot.docs.map(doc => {
            return new Promise<void>((resolve, reject) => {
              ...
              let serviceIsServingInForum = function () {
                return new Promise<void>((resolve, reject) => {
                  const registrantRef = that.afs.collection<Registrant>(`users/${forum.uid}/forums/${forum.forumId}/registrants`).ref.where('serviceId', '==', serviceId);
