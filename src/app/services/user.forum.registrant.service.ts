...
export class UserForumRegistrantService {
  ...
  public delete(parentUserId: string, forumId: string, serviceId: string): Promise<void>{
    return new Promise((resolve, reject) => {
      this.afs.collection<Registrant>(`users/${parentUserId}/forums/${forumId}/registrants`).ref.where('serviceId', '==', serviceId).get().then(registrantSnapshot => {
        if (registrantSnapshot.size > 0){
          ...
          if (registrant.default == true){
            this.afs.collection<Registrant>(`users/${parentUserId}/forums/${forumId}/registrants`).ref.orderBy('creationDate', 'desc').where('uid', '==', registrant.uid).where('default', '==', false).get().then(newDefaultRegistrantSnapshot => {
